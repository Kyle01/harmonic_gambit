class_name RealmNodeButton
extends Control

## Clickable realm-map node visualized as a colored disc using the
## RegionDef.icon_color of whatever region this node holds. Mirrors
## MapNodeButton (the per-region equivalent) but the fill color comes
## from the region rather than an encounter-kind table.

signal node_pressed(id: int)

const RADIUS: float = 22.0
const HIT_RADIUS: float = 28.0

const _OUTLINE_CURRENT: Color = Color(1.0, 1.0, 1.0, 1.0)
const _OUTLINE_AVAILABLE: Color = Color(0.95, 0.93, 0.84, 0.95)
const _OUTLINE_VISITED: Color = Color(0.70, 0.66, 0.58, 0.5)
const _OUTLINE_LOCKED: Color = Color(0.10, 0.10, 0.12, 0.6)

var node_id: int = -1
var fill_color: Color = Color.WHITE
var is_current: bool = false
var is_available: bool = false
var is_visited: bool = false


func setup(node: RealmNode, region: RegionDef) -> void:
	node_id = node.id
	if region != null:
		fill_color = region.icon_color
	else:
		fill_color = Color.WHITE
	# No tooltip: the player discovers a region's identity through
	# icon color and play, not by hovering. Same design principle as
	# the no-role-tags rule for characters.
	custom_minimum_size = Vector2(HIT_RADIUS * 2.0, HIT_RADIUS * 2.0)
	size = custom_minimum_size
	mouse_filter = Control.MOUSE_FILTER_STOP
	queue_redraw()


func set_state(current: bool, available: bool, visited: bool) -> void:
	is_current = current
	is_available = available
	is_visited = visited
	queue_redraw()


func _gui_input(event: InputEvent) -> void:
	if not is_available or is_current:
		return
	if event is InputEventMouseButton:
		var mb: InputEventMouseButton = event
		if mb.button_index == MOUSE_BUTTON_LEFT and mb.pressed:
			var local: Vector2 = mb.position - (size * 0.5)
			if local.length() <= HIT_RADIUS:
				node_pressed.emit(node_id)
				accept_event()


func _draw() -> void:
	var center: Vector2 = size * 0.5
	var fill: Color = fill_color
	var outline: Color = _outline_for_state()
	if is_visited and not is_current:
		fill = fill.darkened(0.35)
	if not is_available and not is_current and not is_visited:
		fill = fill.darkened(0.55)
	draw_circle(center, RADIUS + 3.0, outline)
	draw_circle(center, RADIUS, fill)


func _outline_for_state() -> Color:
	if is_current:
		return _OUTLINE_CURRENT
	if is_available:
		return _OUTLINE_AVAILABLE
	if is_visited:
		return _OUTLINE_VISITED
	return _OUTLINE_LOCKED
