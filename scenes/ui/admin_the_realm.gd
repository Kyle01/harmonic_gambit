...
##
## active_realm is a static var so it persists across scene changes back
...
## Persistent realm for the current run. Set on first entry; cleared
## when the player exits back to admin hub. Surviving across scene
## changes is what lets RegionRunTest hand control back to us.
static var active_realm: Realm = null
...
func _ready() -> void:
	...
	_load_catalog()
	if active_realm == null:
		_plan_new_realm()
	else:
		title_label.text = active_realm.def.display_name.to_upper()
...
func _plan_new_realm() -> void:
	...
	active_realm = RealmPlanner.plan(def, regions, stream)
	title_label.text = def.display_name.to_upper()
	EventBus.realm_built.emit(active_realm)


func _apply_background() -> void:
	if active_realm != null and active_realm.def.background != null:
		background_rect.texture = active_realm.def.background


func _install_controller() -> void:
	_controller = RealmRunController.new()
	_controller.set_realm(active_realm)
...
func _build_map() -> void:
	if active_realm == null:
		return
	for node: RealmNode in active_realm.nodes.values():
...
func _refresh_node_states() -> void:
	...
	for option: RealmNode in active_realm.available_neighbors():
	...
	for id: int in _node_buttons:
		...
		(
			btn
			. set_state(
				id == active_realm.current_node_id,
				available_ids.has(id),
				active_realm.visited_ids.has(id) and id != active_realm.current_node_id,
			)
		)


func _refresh_current_label() -> void:
	if active_realm == null or current_label == null:
		return
	var node: RealmNode = active_realm.current_node()
	if node == null:
		current_label.text = ""
		return
	var region: RegionDef = _catalog_by_id.get(node.region_id, null)
...
func _redraw_edges() -> void:
	...
	for option: RealmNode in active_realm.available_neighbors():
		available_ids[option.id] = true
	var current_id: int = active_realm.current_node_id
	for node: RealmNode in active_realm.nodes.values():
		for next_id: int in node.next_ids:
			var line: Line2D = Line2D.new()
			line.add_point(node.position)
			line.add_point(active_realm.nodes[next_id].position)
			var is_available: bool = node.id == current_id and available_ids.has(next_id)
			var both_visited: bool = (
				active_realm.visited_ids.has(node.id) and active_realm.visited_ids.has(next_id)
			)
			if is_available:
