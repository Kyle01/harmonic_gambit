...
const REGIONS_DIR: String = "res://resources/regions"


static func get_all() -> Array[RegionDef]:
	var results: Array[RegionDef] = []
	var dir: DirAccess = DirAccess.open(REGIONS_DIR)
	if dir == null:
		push_warning("RegionCatalog: cannot open %s" % REGIONS_DIR)
		return results
	dir.list_dir_begin()
	var file_name: String = dir.get_next()
	while file_name != "":
		if not dir.current_is_dir() and _is_resource_file(file_name):
			var path: String = "%s/%s" % [REGIONS_DIR, file_name]
...
static func _by_display_name(a: RegionDef, b: RegionDef) -> bool:
