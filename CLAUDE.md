...
- **One PR, one digestible unit of value.** Each PR should land something a reviewer can hold in their head — a feature, a fix, a refactor with a clear motivation. Don't bundle unrelated drive-bys; split them into separate PRs.
...
- **Fill out the cover page.** Every PR needs a Summary (what changed and why, including design motivation for non-trivial work) and a Test plan (how it was verified — lint, headless import, MCP screenshot, manual play). Blank or one-line PR bodies are not acceptable.
